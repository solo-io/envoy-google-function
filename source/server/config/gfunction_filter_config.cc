...
public:
  Envoy::Server::Configuration::HttpFilterFactoryCb createFilterFactory(
    ...
    return [&context, cb, access_key, secret_key,
            functions](Http::FilterChainFactoryCallbacks &callbacks) -> void {
      auto filter = new Http::GfunctionFilter(
          context.clusterManager(), std::move(cb), std::move(access_key),
          std::move(secret_key), std::move(functions));
